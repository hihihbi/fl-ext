...
func (t *MqttTrigger) Start() error {
	...
	if err != nil {
		fmt.Printf("Error converting \"cleansess\" to a boolean ", err.Error())
		return err
	}
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		fmt.Printf("Received msg:", payload)
		handler, found := t.topicToHandler[topic]
		if found {
			t.RunHandler(handler, payload)
		} else {
			fmt.Printf("handler for topic '%s' not found", topic)
		}
	})
	...
	if err != nil {
		fmt.Printf("Error converting \"qos\" to an integer ", err.Error())
		return err
	}
	...
	for _, handler := range t.handlers {
		...
		if token := t.client.Subscribe(topic, byte(i), nil); token.Wait() && token.Error() != nil {
			fmt.Printf("Error subscribing to topic %s: %s", topic, token.Error())
			return token.Error()
		} else {
			fmt.Printf("Subscribed to topic: %s, will trigger handler: %s", topic, handler)
...
func (t *MqttTrigger) Stop() error {
	...
	for _, handlerCfg := range t.config.Handlers {
		fmt.Printf("Unsubscribing from topic: ", handlerCfg.GetSetting("topic"))
		if token := t.client.Unsubscribe(handlerCfg.GetSetting("topic")); token.Wait() && token.Error() != nil {
			fmt.Printf("Error unsubscribing from topic %s: %s", handlerCfg.Settings["topic"], token.Error())
		}
	}
...
func (t *MqttTrigger) publishMessage(topic string, message string) {
	...
	fmt.Printf("ReplyTo topic: ", topic)
	fmt.Printf("Publishing message: ", message)
	...
	if len(topic) == 0 {
		fmt.Printf("Invalid empty topic to publish to")
		return
	}
	...
	if !sent {
		// Timeout occurred
		fmt.Printf("Timeout occurred while trying to publish to topic '%s'", topic)
		return
	}
}